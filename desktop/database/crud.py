...
def insert_test_data():
    ...
    requisition_list3 = models.RequisitionList(id_worker=3,storage_name="Ларец", date=datetime.datetime.utcnow().replace(microsecond=0), status="В процессе")
    ...
    ingredient_storage1 = models.IngredientStorage(id_ingredient=1,delivery_date=datetime.datetime.utcnow().replace(microsecond=0),
                                                   id_request=1, valid_until=datetime.datetime.utcnow().replace(microsecond=0), weight=10,quantity=300)
    ingredient_storage2 = models.IngredientStorage(id_ingredient=2,delivery_date=datetime.datetime.utcnow().replace(microsecond=0),
                                                   id_request=2, valid_until=datetime.datetime.utcnow().replace(microsecond=0), weight=1,quantity=654)
    ingredient_storage3 = models.IngredientStorage(id_ingredient=3,delivery_date=datetime.datetime.utcnow().replace(microsecond=0),
                                                   id_request=3, valid_until=datetime.datetime.utcnow().replace(microsecond=0), weight=3,quantity=700)
